        ...
        *,
        ...
        def gsync(
            ...
            for j in search(
                ...
                print(
                    f"{name} has searched for '{query}' and it returned {j}"
                )
                return j

        async with ctx.typing():
            gasync = await self.client.loop.run_in_executor(
                ThreadPoolExecutor(),
                gsync,
            )
            await ctx.send(
    ...
    async def wiki(
        ...
        *,
        ...
        if (
            ...
            await ctx.send(
                "You must provide a search term, {ctx.message.author.mention}!"
            )
        c = wikipedia.summary(
            query
        )
